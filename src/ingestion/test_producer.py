...
def create_test_data(file_path: str, num_transactions: int = 50):
    ...
    print(
        f"- Fraud transactions: {fraud_count} ({fraud_count/num_transactions*100:.1f}%)"
    )
...
def test_producer_with_kafka():
    ...
    try:
        ...
        pytest.skip(f"Kafka integration test failed: {e} (Kafka not available)")
    finally:
        # Clean up
        if os.path.exists(test_file):
            os.remove(test_file)
...
def main():
    ...
    if success_basic and success_kafka:
        ...
        print("\nBasic tests passed, but Kafka integration failed.")
