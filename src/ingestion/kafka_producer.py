...
class TransactionProducer:
    ...
    def _create_producer(self) -> KafkaProducer:
        ...
        try:
            ...
            self.logger.info(
                f"Successfully connected to Kafka broker(s): {self.kafka_broker}"
            )
            return producer
    ...
    def _load_transaction_data(self) -> pd.DataFrame:
        ...
        try:
            ...
            if self.use_time_column and "Time" in df.columns:
                ...
                self.logger.info(
                    "Sorted transactions by Time column for realistic simulation"
                )
            ...
            self.logger.error(
                "Please ensure the transaction data file exists or download it first"
            )
            raise
        except Exception as e:
    ...
    def _create_transaction_message(
        self, row: pd.Series, transaction_id: int
    ) -> Dict[str, Any]:
    ...
    def _send_message(self, transaction: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            future = self.producer.send(
                topic=self.topic_name, key=key, value=transaction
            )
    ...
    def _calculate_delay(
        self, current_row: pd.Series, next_row: Optional[pd.Series]
    ) -> float:
    ...
    def start_streaming(self):
        ...
        try:
            ...
            self.logger.info(
                f"Starting to stream transactions to topic: {self.topic_name}"
            )
            ...
            while self.running:
                ...
                for idx, row in df.iterrows():
                    ...
                    transaction = self._create_transaction_message(
                        row, self.messages_sent + 1
                    )
                    ...
                    if self._send_message(transaction):
                        ...
                        if self.messages_sent % 100 == 0:
                            fraud_status = (
                                "FRAUD" if transaction.get("Class") == 1 else "NORMAL"
                            )
                            self.logger.info(
                ...
                self.logger.info(
                    f"Completed iteration {iteration}, sent {self.messages_sent} total messages"
                )
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Kafka Producer for Credit Card Transactions"
    )
    ...
    parser.add_argument(
        "--topic", type=str, help="Kafka topic name (overrides environment variable)"
    )
    ...
    parser.add_argument(
        "--loop", action="store_true", help="Loop through the dataset continuously"
    )
