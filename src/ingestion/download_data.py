...
def download_from_s3(
    bucket_name: str, key: str, output_path: str, logger: logging.Logger
) -> bool:
...
def create_sample_data(output_path: str, logger: logging.Logger) -> bool:
    ...
    try:
        ...
        fraud_indices = np.random.choice(
            n_samples, size=int(n_samples * 0.002), replace=False
        )
        ...
        logger.info(
            f"Fraud transactions: {int(df['Class'].sum())} ({df['Class'].mean()*100:.3f}%)"
        )
...
def main():
    ...
    if create_sample_data(output_path, logger):
        ...
        logger.warning(
            "Note: This is sample data, not the real Credit Card Fraud Detection dataset"
        )
