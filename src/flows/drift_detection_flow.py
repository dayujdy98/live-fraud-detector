...
def preprocess_data(df: pd.DataFrame, data_type: str = "reference") -> pd.DataFrame:
    ...
    if len(df_processed) < initial_rows:
        logger.info(
            f"Removed {initial_rows - len(df_processed)} duplicate rows from {data_type} data"
        )

    logger.info(
        f"Preprocessed {data_type} data: {len(df_processed)} rows, {len(df_processed.columns)} columns"
    )
    return df_processed
...
def prepare_data_for_analysis(
    ...
    if "prediction" in current_data.columns:
        ...
        logger.warning(
            "No 'prediction' column found in current data. Using dummy target."
        )
...
def run_drift_analysis(
    reference_data: pd.DataFrame, current_data: pd.DataFrame, report_path: str
) -> dict:
    ...
    try:
        ...
        target_drift = (
            report_dict.get("target_drift", {})
            .get("data", {})
            .get("metrics", {})
            .get("target_drift", False)
        )

        # Get column drift details
        column_drifts = {}
        if "data_drift" in report_dict:
            for metric_name, metric_data in report_dict["data_drift"]["data"][
                "metrics"
            ].items():
                if metric_name.startswith("column_") and metric_name.endswith("_drift"):
                    column_name = metric_name.replace("column_", "").replace(
                        "_drift", ""
                    )
                    column_drifts[column_name] = metric_data.get(
...
def save_drift_summary(
    drift_summary: dict, summary_path: str = "reports/drift_summary.json"
):
...
def check_data_quality(current_data: pd.DataFrame) -> dict:
    ...
    quality_metrics = {
        ...
        "categorical_columns": len(
            current_data.select_dtypes(include=["object"]).columns
        ),
    }

    logger.info(
        f"Data quality check completed. Total rows: {quality_metrics['total_rows']}"
    )
...
def drift_detection_flow(
    ...
    try:
        ...
        if len(current_df) < min_data_points:
            logger.warning(
                f"Insufficient data points for analysis. Required: {min_data_points}, Available: {len(current_df)}"
            )
            return {
        ...
        ref_analysis, current_analysis = prepare_data_for_analysis(
        ...
        if drift_summary["drift_detected"]:
            logger.warning(
                "ALERT: Data drift detected! Consider model retraining or investigation."
            )
            ...
            logger.info(
                "No significant drift detected. Model appears to be performing well."
            )
