...
def create_sample_inference_log(output_path: str = "/app/data/inference_log.csv"):
    ...
    features['prediction'] = np.random.beta(2, 8, n_samples)  # Different distribution than Class
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            if 'data_quality' in result:
                ...
                print(f"Data quality - Duplicate rows: {quality.get('duplicate_rows', 0)}")
            
            # Check if files were created
            if os.path.exists(report_path):
                print(f"✅ Drift report created: {report_path}")
            else:
                print(f"❌ Drift report not created")
            
            if os.path.exists(summary_path):
                print(f"✅ Drift summary created: {summary_path}")
            else:
                print(f"❌ Drift summary not created")
...
def test_with_real_data():
    ...
    if not os.path.exists(real_ref_path):
        print(f"❌ Real reference data not found: {real_ref_path}")
        return None
    
    if not os.path.exists(real_current_path):
        print(f"❌ Real inference log not found: {real_current_path}")
        return None
...
if __name__ == "__main__":
    ...
    if real_result:
        ...
        print("⚠️  Real data test: SKIPPED (no real data available)")
