...
def generate_fraudulent_transaction():
    ...
    for i in range(1, 29):
        if i in [1, 2, 3]:  # Features often associated with fraud
            ...
            transaction[f'V{i}'] = round(random.uniform(2, 4), 6)  # High positive values
...
def send_to_kafka(producer, topic, transaction):
    ...
    try:
        ...
        logger.info(f"Sent transaction {transaction['transaction_id']} to topic {topic}")
...
def main():
    parser = argparse.ArgumentParser(description='Generate test transaction data for Kafka')
    ...
    logger.info(f"Starting test data generation:")
